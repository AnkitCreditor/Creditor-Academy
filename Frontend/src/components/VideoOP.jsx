...
const CourseContentSnapshot = () => {
  return (
    <>
      ...
      <section style={{ background: "#f9fafb", padding: "80px 5%", fontFamily: "'Segoe UI', sans-serif" }}>
        <h2 style={{ textAlign: "center", fontSize: "2.5rem", marginBottom: "20px", color: "#0ea5e9" }}>
          <span style={{ color: "#236fa1" }}><strong>Course Content Snapshot</strong></span>
        </h2>
        <p style={{ textAlign: "center", fontSize: "1.1rem", color: "#475569", maxWidth: "720px", margin: "auto" }}>
          Just a glimpse of the strategic documents and private power tools you’ll build inside.
        </p>

        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "30px", marginTop: "60px" }}>
          {contentBlocks.map((block, index) => (
            ...
            >
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: "1.2rem", color: "#0f172a" }}>{block.title}</h3>
                <p style={{ marginTop: "6px", color: "#64748b" }}>{block.description}</p>
              </div>
            </div>
          ))}
        </div>
