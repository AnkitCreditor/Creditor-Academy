...
const BecomePrivateCourse = () => {
  ...
  const getMaxSlides = () => {
    ...
    if (width >= 1024) return Math.max(0, cards.length - 3); // 3 cards visible
    if (width >= 640) return Math.max(0, cards.length - 2); // 2 cards visible
    return Math.max(0, cards.length - 1); // 1 card visible
  };
        ...
        <div style={{
          ...
          <div 
            ...
            style={{
              ...
              transform: `translateX(-${currentSlide * (100 / (window.innerWidth >= 1024 ? 3 : window.innerWidth >= 640 ? 2 : 1))}%)`
            }}
          >
            {cards.map((item, index) => (
              <motion.div
                ...
                whileHover={{ scale: 1.02 }}
                style={{
                  flex: window.innerWidth >= 1024 ? '0 0 33.333%' : window.innerWidth >= 640 ? '0 0 50%' : '0 0 100%',
                  maxWidth: window.innerWidth >= 1024 ? '33.333%' : window.innerWidth >= 640 ? '50%' : '100%',
                  width: '320px',
                  background: '#fff',
                  borderRadius: '20px',
                  overflow: 'hidden',
